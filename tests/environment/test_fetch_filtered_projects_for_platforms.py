...
from tests.mocks import MockGitPlatform, MockLogger
...
class TestFetchFilteredProjectsForPlatforms(unittest.TestCase):
    ...
    def test_missing_platform_id(self) -> None:
        ...
        with self.assertRaises(ValueError):
            fetch_filtered_projects_for_platforms([platform_without_id], self.mock_logger)
        self.assertTrue(self.mock_logger.has_logged("Platform ID missing for platform: mock. Skipping.."))

    def test_one_platform_with_projects(self) -> None:
        result = fetch_filtered_projects_for_platforms([self.platform], self.mock_logger)
    ...
    def test_two_platforms_with_and_without_projects(self) -> None:
        ...
        result = fetch_filtered_projects_for_platforms([self.platform, platform2], self.mock_logger)
        self.assertEqual(len(result["mock_id"]), 2)
        if platform2.id:
    ...
    def test_no_platforms(self) -> None:
        with self.assertRaises(ValueError) as context:
            fetch_filtered_projects_for_platforms([], self.mock_logger)
